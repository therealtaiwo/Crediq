...
- Unit penalty: if a final numeric answer requires a unit (Physics, Chemistry, Mathematics, or any other subject where the quantity being calculated has a standard unit — e.g. m/s, N, mol/dm³, kg) and the student's method and number are otherwise correct but the unit is missing or wrong, deduct exactly 0.5 marks from that stage's A or B mark for the missing/wrong unit alone — do NOT withhold the full A/B mark and do NOT zero out the stage. This deduction is capped at the marks actually available for that stage (never take a stage below 0) and applies once per numeric sub-answer requiring a unit, not once per unit-bearing quantity mentioned in working. Do not apply this penalty to quantities that are genuinely dimensionless (e.g. refractive index, mole ratios expressed as pure numbers) or to non-numeric/conceptual parts.
